...
import { Pendingorder } from "../Api/User";
...
const Pendingorder_ = () => {
  ...
  const getalldata = async () => {
    ...
    var myorderdata = await Pendingorder({ id: store_id });
    setrows(myorderdata);
  };
  const processingorder = async (e) => {
    var updatedata = await UpdateOrders({
      id: e.target.id,
      status: "Processing",
    });
  ...
  const exportoption = () => {
    ...
    XLSX.writeFile(wb, "pendingorder.xlsx");
  };
    ...
    <div className="flex ">
      ...
      <div>
        ...
        <div className="col-span-2 p-8 Products">
          <div className="flex ">
            <h1 className=" p-3 mr-4 mb-4 ">Pending Orders</h1>
            {rows.length !== 0 ? (
              <button
                      ...
                      .map((row, index) => {
                        return (
                          ...
                          >
                            ...
                            <TableCell  id={row.order.id}>{row.user.name}</TableCell>
                            <TableCell  id={row.order.id}>{row.product.name}</TableCell>
                            <TableCell  id={row.order.id}>{row.order.Quantity}</TableCell>
                            <TableCell  id={row.order.id}>{row.order.price}</TableCell>
                            ...
                            <TableCell  id={row.order.id}>{row.order.status}</TableCell>
                            <TableCell align="center"  id={row.order.id}>
                              <button
